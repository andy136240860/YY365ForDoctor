...
- (void)initContentView
{
    ...
    _avatarView = [[UIImageView alloc] initWithFrame:CGRectMake((viewWidth - imageWidth) / 2, 60, imageWidth, imageWidth)];
    ...
    UILabel *avatarLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_avatarView.frame) + 10, viewWidth, 15)];
    avatarLabel.text = @"点击更换头像";
    avatarLabel.textAlignment = NSTextAlignmentCenter;
    avatarLabel.font = [UIFont systemFontOfSize:12];
    avatarLabel.textColor = kYellowColor;
    [_contentScrollView addSubview:avatarLabel];
    ...
    NSArray *titleArray = @[@"姓名",@"昵称",@"性别",@"年龄",@"电话"];
    NSArray *viewArray = @[_nameField,_nickField,_genderField,_ageField,_phoneField];
    ...
    for (int i = 0; i < titleArray.count; i ++) {
        ...
        textField.frame = CGRectMake(100, originY, viewWidth - 100 - padding, cellHeight);
...
- (void)fillData
{
    ...
    _ageField.text = self.user.cellPhone;
...
- (void)textField:(UITextField *)textField didChange:(NSString *)newString
{
    if (textField == _nameField) {
        ...
        self.tempMember.nickName = newString;
    }
...
- (void)confirmAction
{
    ...
    self.tempMember.nickName = _nickField.text;
